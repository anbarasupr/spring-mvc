...
/*
 * 1) Set below command in catalina batch file under tomcat server. 
 * set "JAVA_OPTS=%JAVA_OPTS% -Djava.security.auth.login.config==%CATALINA_BASE%/conf/jaas.config"

 * 2) Move jaas.config file in tomcat/conf folder which is given in resources/lib of this project*
 *
 * */
public class CustomLoginModuleTest implements LoginModule {
	...
	public boolean login() throws LoginException {
		...
		try {
			...
			String password = String.valueOf(((PasswordCallback) callbacks[1])
					.getPassword());
			System.out.println("JAAS USER:"+name+", Password:"+password);
